...
class UnpairedPreprocessor:
    ...
    def fit(self, data: UnpairedOmicsData, rna_train_idx, prot_train_idx):
        ...
        self.prot_features_ = (
            prot_i.var(axis=0).sort_values(ascending=False).head(self.n_prot).index
        )
...
def _pca_ridge_predict(Xtr, Ytr, Xte, n_components=3, alpha=10.0):
    k = max(1, min(n_components, len(Xtr) - 1, Xtr.shape[1]))
...
def _pls_predict(Xtr, Ytr, Xte, n_components=2):
    k = max(1, min(n_components, len(Xtr) - 1, Xtr.shape[1], Ytr.shape[1]))
    return PLSRegression(n_components=k, scale=False, max_iter=1000).fit(Xtr, Ytr).predict(Xte)


def _cognate_ridge_predict(
    Xtr, Ytr, Xte, protein_names, rna_names, cognate, alpha=5.0, fallback=None
):
    """Per protein ridge on the corresponding transcript at CONDITION-MEAN level."""
    rna_col = {g: i for i, g in enumerate(rna_names)}
    pred = np.empty((len(Xte), len(protein_names)), float)
    if fallback is None:
        fallback = np.tile(Ytr.mean(axis=0), (len(Xte), 1))
    for j, p in enumerate(protein_names):
        g = cognate.get(str(p))
        i = rna_col.get(str(g), None)
        if i is None:
            pred[:, j] = fallback[:, j]
            continue
...
def _fold_preprocessors(data, splits, pre_kwargs):
    pres, psets = [], []
    for s in splits:
...
def run_unpaired_cv(
    ...
    """Evaluate all models on condition means without ever pairing replicates.

    Scoring is in the transformed protein scale (normally log2 intensity). For
    each protein, pooled R2 is calculated across held-out conditions against the
    training-condition mean from the corresponding fold.
    """
    splits = leave_condition_out(data) if splits is None else splits
    pre_kwargs = pre_kwargs or {}
    ae_kwargs = ae_kwargs or {}
    pres, protein_names = _fold_preprocessors(data, splits, pre_kwargs)
    if not protein_names:
        raise ValueError("no protein target survives preprocessing in every fold")

    model_names = ["mean", "design_only", "cognate_ridge", "pca_ridge", "pls", "condition_aligned_ae"]
    ...
    for fold, (s, pre) in enumerate(zip(splits, pres)):
        ...
        if len(test_conds) != 1:
            # leave-treatment-out may hold multiple cells; score each condition separately
            pass
        ...
        pred_std = {}
        pred_std["mean"] = np.tile(_mean_predict(Ytr), (len(test_conds), 1))
        pred_std["design_only"] = _design_predict(Ytr, train_conds, test_conds, design)
        pred_std["pca_ridge"] = _pca_ridge_predict(
            Xtr, Ytr, Xte, n_components=pca_components
        )
        pred_std["pls"] = _pls_predict(Xtr, Ytr, Xte, n_components=pls_components)
        pred_std["cognate_ridge"] = _cognate_ridge_predict(
            Xtr, Ytr, Xte, protein_names, list(pre.rna_features_), data.cognate,
            fallback=pred_std["mean"],
        )
        ...
        # aggregate independently for every held condition
        ae_cond_pred = []
        rtest_cond = data.rna_conditions[s["rna_test"]]
        for c in test_conds:
            ae_cond_pred.append(ae_sample_pred[rtest_cond == c].mean(axis=0))
        pred_std["condition_aligned_ae"] = np.asarray(ae_cond_pred)

        # back-transform fold-specific standardisation to a common log-protein scale
        ...
        for row_i, cond in enumerate(test_conds):
            ...
            for name in model_names:
                if name == "mean":
                    # Make the reference model exactly identical to the R2 denominator.
                    pp = baseline.copy()
                else:
                    pp = pre.inverse_protein(pred_std[name][row_i:row_i + 1], protein_names)[0]
                pred_rows[name].append(pp)
                sse[name] += (y - pp) ** 2

        # fold summary: relative to this fold's train-condition mean
        denom = np.maximum((Yte_log - baseline) ** 2, 1e-12)
        for name in model_names:
            pp = pre.inverse_protein(pred_std[name], protein_names)
            r2_point = 1.0 - ((Yte_log - pp) ** 2) / denom
            fold_rows.append(
                dict(
                    fold=fold,
                    ...
                    median_relative_r2=float(np.nanmedian(r2_point)),
                    rmse_log=float(np.sqrt(np.nanmean((Yte_log - pp) ** 2))),
                )
            )
        if verbose:
...
def permutation_null(
    ...
    **cv_kwargs,
    ...
    """Shuffle protein CONDITION labels, preserving independent protein replicates."""
    rng = np.random.default_rng(seed)
    out = []
    original = data.prot_meta.copy()
    unique = pd.unique(data.prot_conditions)
    for i in range(n_perm):
        d = copy.copy(data)
        pm = original.copy()
        mapping = dict(zip(unique, rng.permutation(unique)))
        # Replace treatment/timepoint by the labels of a permuted condition.
        cond_meta = _condition_metadata(data)
        new_rows = []
        for old in data.prot_conditions:
            new = mapping.get(old, old)
            vals = cond_meta.loc[new]
        ...
        d.prot_meta = pm
        _, oof = run_unpaired_cv(d, verbose=False, **cv_kwargs)
